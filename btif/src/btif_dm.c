...
static void bond_state_changed(bt_status_t status, bt_bdaddr_t *bd_addr, bt_bond_state_t state)
{
    ...
    BTIF_TRACE_ERROR3("%s: state=%d prev_state=%d", __FUNCTION__, state, pairing_cb.state);
                                          ...
                                          DEV_CLASS dev_class, tBT_DEVICE_TYPE device_type)
{
    ...
    if ( cod == 0) {
        BTIF_TRACE_ERROR1("%s():cod is 0, set as unclassified", __FUNCTION__);
...
static void btif_dm_cb_hid_remote_name(tBTM_REMOTE_DEV_NAME *p_remote_name)
{
    BTIF_TRACE_ERROR3("%s: status=%d pairing_cb.state=%d", __FUNCTION__, p_remote_name->status, pairing_cb.state);
...
static void search_devices_copy_cb(UINT16 event, char *p_dest, char *p_src)
{
    ...
    BTIF_TRACE_ERROR2("%s: event=%s", __FUNCTION__, dump_dm_search_event(event));
...
static void btif_dm_pin_req_evt(tBTA_DM_PIN_REQ *p_pin_req)
{
    ...
    if (pairing_cb.is_local_initiated)
    {
        ...
        {
            BTIF_TRACE_ERROR1("%s()cod matches for auto pair", __FUNCTION__);
            /*  Check if this device can be auto paired  */
            if ((btif_storage_is_device_autopair_blacklisted(&bd_addr) == FALSE) &&
                (pairing_cb.autopair_attempts == 0))
            {
                BTIF_TRACE_ERROR1("%s() Attempting auto pair", __FUNCTION__);
                pin_code.pin[0] = 0x30;
                pin_code.pin[1] = 0x30;
                pin_code.pin[2] = 0x30;
                pin_code.pin[3] = 0x30;
...
static void btif_dm_auth_cmpl_evt (tBTA_DM_AUTH_CMPL *p_auth_cmpl)
{
    ...
    if ( (p_auth_cmpl->success == TRUE) && (p_auth_cmpl->key_present) )
    {
        ...
        {
            bt_status_t ret;
            BTIF_TRACE_ERROR3("%s: Storing link key. key_type=0x%x, is_temp=%d",
        ...
        {
            BTIF_TRACE_ERROR3("%s: Temporary key. Not storing. key_type=0x%x, is_temp=%d",
                __FUNCTION__, p_auth_cmpl->key_type, pairing_cb.is_temp);
        }
    }
    ...
    else
    {
        ...
        switch(p_auth_cmpl->fail_reason)
        {
            ...
            case HCI_ERR_INSUFFCIENT_SECURITY:
                BTIF_TRACE_ERROR1(" %s() Authentication fail ", __FUNCTION__);
                if (pairing_cb.autopair_attempts  == 1)
                {
                    BTIF_TRACE_ERROR1("%s(): Adding device to blacklist ", __FUNCTION__);
                    ...
                    BTIF_TRACE_ERROR1("%s() auto pair failed. Reinitiate Bond", __FUNCTION__);
...
static void btif_dm_search_devices_evt (UINT16 event, char *p_param)
{
    ...
    switch (event)
    {
        case BTA_DM_DISC_RES_EVT:
        {
            ...
            BTIF_TRACE_ERROR1("%s Event : BTA_DM_DISC_RES_EVT ", __FUNCTION__);
            p_search_data = (tBTA_DM_SEARCH *)p_param;
            ALOGD("BTA_DM_DISC_RES_EVT GOOOOO -> Remote name update len=%d", strlen((const char *) p_search_data->disc_res.bd_name));
            /* Remote name update */
        ...
        case BTA_DM_INQ_RES_EVT:
        {
            ALOGD("BTA_DM_INQ_RES_EVT GOOOOO -> inquiry result");
            BTIF_TRACE_ERROR1("%s Event : BTA_DM_INQ_RES_EVT ", __FUNCTION__);
            /* inquiry result */
            UINT32 cod;
            UINT8 *p_eir_remote_name = NULL;
            bt_bdname_t bdname;
            ...
            BTIF_TRACE_ERROR3("%s() %s device_type = 0x%x\n", __FUNCTION__, bd2str(&bdaddr, &bdstr),
#if (BLE_INCLUDED == TRUE)
            ...
            if (p_search_data->inq_res.p_eir)
            {
                ...
                BTIF_TRACE_ERROR2("%s()EIR BTA services = %08X", __FUNCTION__, (UINT32)services);
        ...
        {
            ALOGD("BTA_DM_DISC_CMPL_EVT GOOOOO");
            BTIF_TRACE_ERROR1("%s Event : BTA_DM_DISC_CMPL_EVT ", __FUNCTION__);
...
static void btif_dm_search_services_evt(UINT16 event, char *p_param)
{
    ...
    switch (event)
    {
        case BTA_DM_DISC_RES_EVT:
        {
            ...
            BTIF_TRACE_ERROR3("%s:(result=0x%x, services 0x%x)", __FUNCTION__,
            ...
            */
//             if ((pairing_cb.state == BT_BOND_STATE_BONDING) ) {
//                                  BTIF_TRACE_ERROR1("%s A Remote Service SDP done. Call bond_state_changed_cb BONDED",
//                                    __FUNCTION__);
//                 if ( (bdcmp(p_data->disc_res.bd_addr, pairing_cb.bd_addr) == 0) ) {
//                                      BTIF_TRACE_ERROR1("%s  B Remote Service SDP done. Call bond_state_changed_cb BONDED",
//                                    __FUNCTION__);
//                     if ( pairing_cb.sdp_attempts > 0 )
            {
                 BTIF_TRACE_ERROR1("%s C Remote Service SDP done. Call bond_state_changed_cb BONDED",
                                   __FUNCTION__);
                 pairing_cb.sdp_attempts  = 0;
...
//             }
        ...
        case BTA_DM_DISC_BLE_RES_EVT:
             BTIF_TRACE_ERROR2("%s:, services 0x%x)", __FUNCTION__,
             ...
             {
                BTIF_TRACE_ERROR1("%s: Found HOGP UUID",__FUNCTION__);
                bt_property_t prop;
                bt_bdaddr_t bd_addr;
                char temp[256];
                bt_status_t ret;
...
static void btif_dm_upstreams_evt(UINT16 event, char* p_param)
{
    ...
    switch (event)
    {
        ...
        case BTA_DM_LINK_UP_EVT:
            ...
            BTIF_TRACE_ERROR0("BTA_DM_LINK_UP_EVT. Sending BT_ACL_STATE_CONNECTED");
        ...
        case BTA_DM_LINK_DOWN_EVT:
            bdcpy(bd_addr.address, p_data->link_down.bd_addr);
            BTIF_TRACE_ERROR0("BTA_DM_LINK_DOWN_EVT. Sending BT_ACL_STATE_DISCONNECTED");
            HAL_CBACK(bt_hal_cbacks, acl_state_changed_cb, BT_STATUS_SUCCESS,
        ...
        case BTA_DM_BLE_KEY_EVT:
            BTIF_TRACE_ERROR1("BTA_DM_BLE_KEY_EVT key_type=0x%02x ", p_data->ble_key.key_type);
            ...
            if (pairing_cb.state != BT_BOND_STATE_BONDING)
            {
                BTIF_TRACE_ERROR0("Bond state not sent to App so far.Notify the app now");
            ...
            switch (p_data->ble_key.key_type)
            {
                case BTA_LE_KEY_PENC:
                    BTIF_TRACE_ERROR0("Rcv BTA_LE_KEY_PENC");
                    ...
                    for (i=0; i<16; i++)
                    {
                        BTIF_TRACE_ERROR2("pairing_cb.ble.penc_key.ltk[%d]=0x%02x",i,pairing_cb.ble.penc_key.ltk[i]);
                    }
                    for (i=0; i<8; i++)
                    {
                        BTIF_TRACE_ERROR2("pairing_cb.ble.penc_key.rand[%d]=0x%02x",i,pairing_cb.ble.penc_key.rand[i]);
                    }
                    BTIF_TRACE_ERROR1("pairing_cb.ble.penc_key.ediv=0x%04x",pairing_cb.ble.penc_key.ediv);
                    BTIF_TRACE_ERROR1("pairing_cb.ble.penc_key.sec_level=0x%02x",pairing_cb.ble.penc_key.sec_level);
                    BTIF_TRACE_ERROR1("pairing_cb.ble.penc_key.key_size=0x%02x",pairing_cb.ble.penc_key.key_size);
                    break;

                case BTA_LE_KEY_PID:
                    BTIF_TRACE_ERROR0("Rcv BTA_LE_KEY_PID");
                    pairing_cb.ble.is_pid_key_rcvd = TRUE;
                    memcpy(pairing_cb.ble.pid_key, p_data->ble_key.key_value.pid_key.irk, 16);
                    for (i=0; i<16; i++)
                    {
                        BTIF_TRACE_ERROR2("pairing_cb.ble.pid_key[%d]=0x%02x",i,pairing_cb.ble.pid_key[i]);
                    }
                    break;

                case BTA_LE_KEY_PCSRK:
                    BTIF_TRACE_ERROR0("Rcv BTA_LE_KEY_PCSRK");
                    ...
                    for (i=0; i<16; i++)
                    {
                        BTIF_TRACE_ERROR2("pairing_cb.ble.pcsrk_key.csrk[%d]=0x%02x",i,pairing_cb.ble.pcsrk_key.csrk[i]);
                    }
                    BTIF_TRACE_ERROR1("pairing_cb.ble.pcsrk_key.counter=0x%08x",pairing_cb.ble.pcsrk_key.counter);
                    BTIF_TRACE_ERROR1("pairing_cb.ble.pcsrk_key.sec_level=0x%02x",pairing_cb.ble.pcsrk_key.sec_level);
                    break;

                case BTA_LE_KEY_LENC:
                    BTIF_TRACE_ERROR0("Rcv BTA_LE_KEY_LENC");
                    ...
                    BTIF_TRACE_ERROR1("pairing_cb.ble.lenc_key.div=0x%04x",pairing_cb.ble.lenc_key.div);
                    BTIF_TRACE_ERROR1("pairing_cb.ble.lenc_key.key_size=0x%02x",pairing_cb.ble.lenc_key.key_size);
                    BTIF_TRACE_ERROR1("pairing_cb.ble.lenc_key.sec_level=0x%02x",pairing_cb.ble.lenc_key.sec_level);
                    break;



                case BTA_LE_KEY_LCSRK:
                    BTIF_TRACE_ERROR0("Rcv BTA_LE_KEY_LCSRK");
                    ...
                    BTIF_TRACE_ERROR1("pairing_cb.ble.lcsrk_key.div=0x%04x",pairing_cb.ble.lcsrk_key.div);
                    BTIF_TRACE_ERROR1("pairing_cb.ble.lcsrk_key.counter=0x%08x",pairing_cb.ble.lcsrk_key.counter);
                    BTIF_TRACE_ERROR1("pairing_cb.ble.lcsrk_key.sec_level=0x%02x",pairing_cb.ble.lcsrk_key.sec_level);
        ...
        case BTA_DM_BLE_SEC_REQ_EVT:
            BTIF_TRACE_ERROR0("BTA_DM_BLE_SEC_REQ_EVT. ");
            btif_dm_ble_sec_req_evt(&p_data->ble_req);
            break;
        case BTA_DM_BLE_PASSKEY_NOTIF_EVT:
            BTIF_TRACE_ERROR0("BTA_DM_BLE_PASSKEY_NOTIF_EVT. ");
            btif_dm_ble_key_notif_evt(&p_data->key_notif);
            break;
        case BTA_DM_BLE_PASSKEY_REQ_EVT:
            BTIF_TRACE_ERROR0("BTA_DM_BLE_PASSKEY_REQ_EVT. ");
            btif_dm_ble_passkey_req_evt(&p_data->pin_req);
            break;
        case BTA_DM_BLE_OOB_REQ_EVT:
            BTIF_TRACE_ERROR0("BTA_DM_BLE_OOB_REQ_EVT. ");
            break;
        case BTA_DM_BLE_LOCAL_IR_EVT:
            BTIF_TRACE_ERROR0("BTA_DM_BLE_LOCAL_IR_EVT. ");
        ...
        case BTA_DM_BLE_LOCAL_ER_EVT:
            BTIF_TRACE_ERROR0("BTA_DM_BLE_LOCAL_ER_EVT. ");
        ...
        case BTA_DM_BLE_AUTH_CMPL_EVT:
            BTIF_TRACE_ERROR0("BTA_DM_BLE_KEY_EVT. ");
...
static void bte_search_devices_evt(tBTA_DM_SEARCH_EVT event, tBTA_DM_SEARCH *p_data)
{
    ...
    BTIF_TRACE_ERROR3("%s event=%s param_len=%d", __FUNCTION__, dump_dm_search_event(event), param_len);
                               ...
                               uint8_t pin_len, bt_pin_code_t *pin_code)
{
    ...
    if (pairing_cb.is_le_only)
    {
        ...
        BTIF_TRACE_ERROR1("btif_dm_pin_reply: passkey: %d", passkey);
...
void btif_dm_set_oob_for_io_req(tBTA_OOB_DATA  *p_oob_data)
{
    ...
    BTIF_TRACE_ERROR1("btif_dm_set_oob_for_io_req *p_oob_data=%d", *p_oob_data);
}
...
void btif_dm_load_local_oob(void)
{
    ...
    BTIF_TRACE_ERROR1("btif_dm_load_local_oob prop_oob = %s",prop_oob);
    if (prop_oob[0] != '3')
    {
#if (BTM_OOB_INCLUDED == TRUE)
        if (oob_cb.sp_c[0] == 0 && oob_cb.sp_c[1] == 0 &&
            oob_cb.sp_c[2] == 0 && oob_cb.sp_c[3] == 0 )
        {
            BTIF_TRACE_ERROR0("btif_dm_load_local_oob: read OOB, call BTA_DmLocalOob()");
            BTA_DmLocalOob();
        }
#else
...
void btif_dm_proc_loc_oob(BOOLEAN valid, BT_OCTET16 c, BT_OCTET16 r)
{
    ...
    char *path = NULL;
    char prop_oob[PROPERTY_VALUE_MAX];
    BTIF_TRACE_ERROR1("btif_dm_proc_loc_oob: valid=%d", valid);
    if (oob_cb.sp_c[0] == 0 && oob_cb.sp_c[1] == 0 &&
        oob_cb.sp_c[2] == 0 && oob_cb.sp_c[3] == 0 &&
        valid)
    {
        BTIF_TRACE_ERROR0("save local OOB data in memory");
        memcpy(oob_cb.sp_c, c, BT_OCTET16_LEN);
        memcpy(oob_cb.sp_r, r, BT_OCTET16_LEN);
        property_get("service.brcm.bt.oob", prop_oob, "3");
        BTIF_TRACE_ERROR1("btif_dm_proc_loc_oob prop_oob = %s",prop_oob);
        if (prop_oob[0] == '1')
            path = path_a;
        ...
        if (path)
        {
            ...
            if (fp == NULL)
            {
                BTIF_TRACE_ERROR1("btif_dm_proc_loc_oob: failed to save local OOB data to %s", path);
            }
            else
            {
                BTIF_TRACE_ERROR1("btif_dm_proc_loc_oob: save local OOB data into file %s",path);
...
BOOLEAN btif_dm_proc_rmt_oob(BD_ADDR bd_addr,  BT_OCTET16 p_c, BT_OCTET16 p_r)
{
    ...
    property_get("service.brcm.bt.oob", prop_oob, "3");
    BTIF_TRACE_ERROR1("btif_dm_proc_rmt_oob prop_oob = %s",prop_oob);
    if (prop_oob[0] == '1')
        path = path_b;
    else if (prop_oob[0] == '2')
        path = path_a;
    if (path)
    {
        ...
        if (fp == NULL)
        {
            BTIF_TRACE_ERROR1("btapp_dm_rmt_oob_reply: failed to read OOB keys from %s",path);
            return FALSE;
        }
        else
        {
            BTIF_TRACE_ERROR1("btif_dm_proc_rmt_oob: read OOB data from %s",path);
            fread (p_c , 1 , BT_OCTET16_LEN , fp );
            fread (p_r , 1 , BT_OCTET16_LEN , fp );
            fclose(fp);
        }
        BTIF_TRACE_ERROR0("----btif_dm_proc_rmt_oob: TRUE");
        ...
        BTIF_TRACE_ERROR1("----btif_dm_proc_rmt_oob: peer_bdaddr = %s", t);
...
void    btif_dm_load_ble_local_keys(void)
{
    ...
    {
        ble_local_key_cb.is_er_rcvd = TRUE;
        BTIF_TRACE_ERROR1("%s BLE ER key loaded",__FUNCTION__ );
    }
    ...
    {
        ble_local_key_cb.is_id_keys_rcvd = TRUE;
        BTIF_TRACE_ERROR1("%s BLE ID keys loaded",__FUNCTION__ );
    }
                                   ...
                                   tBTA_BLE_LOCAL_ID_KEYS *p_id_keys)
{
    ...
    BTIF_TRACE_ERROR2("%s  *p_key_mask=0x%02x",__FUNCTION__,   *p_key_mask);
}
...
void btif_dm_ble_sec_req_evt(tBTA_DM_BLE_SEC_REQ *p_ble_req)
{
    ...
    if (pairing_cb.state == BT_BOND_STATE_BONDING)
    {
        BTIF_TRACE_ERROR1("%s Discard security request", __FUNCTION__);
        return;
    }
...
void btif_dm_on_disable()
{
    ...
    if (pairing_cb.state == BT_BOND_STATE_BONDING)
    {
        ...
        BTIF_TRACE_ERROR1("%s: Cancel pending pairing request", __FUNCTION__);
        bdcpy(bd_addr.address, pairing_cb.bd_addr);
